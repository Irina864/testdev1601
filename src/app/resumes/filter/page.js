        ...
        <main className={styles.main}>
          ...
          {isLoading ? (
            ...
            <>
              {isMobile ? (
                !isAsideVisible ? (
                  // Рендер для мобильного, когда aside не виден
                  <div className={styles.container}>
                    <div className={styles.content}>
                      <BackButton
                        isMobile={isMobile}
                        ...
                        onGoBack={goBack}
                        nameLink="Карточка резюме"
                        customClassName="goBack_resumes"
                      />
                      {data.length > 0 ? <ResumeDescription /> : ''}
                    </div>
                  </div>
                ) : (
                  // Рендер для мобильного, когда aside виден
                  <div className={styles.container}>
                  <aside className={styles.aside}>
                    <>
                      {data.length > 0
                        ? resumeList.map((item) => {
                            return <ApplicantCard item={item} key={item.id} onOptionClick={() => setAsideVisible(false)} />;
                          })
                        : ''}
                    </>
                  </aside>
