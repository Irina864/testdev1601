...
const ResumeDescription = () => {
  ...
  useEffect(() => {
    // console.log(isReaction, 'карта', reactionList.length > 0);
    setIsReaction(Boolean(reactionList.find((el) => el.resume === data.id)));
  ...
  const workFormat = () => {
    ...
    if (!data || !data.work_format || data.work_format.length <= 0) {
      return 'любой формат';
    } else {
      return format(data.work_format);
    }
  };
