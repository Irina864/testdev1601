...
const showExperience = (from, to, no_experience = false) => {
  no_experience ? 'Без опыта' : '';
  if (!from && !to && !no_experience) {
    return '';
  }
  // до ...
  if (to && !from && Number(to) === 1) {
    return `До ${to} года`;
  }
  if (to && !from && Number(to) > 1) {
    return `До ${to} лет`;
      ...
      case "Опыт":
        if (selectedFilters.experienceFrom || selectedFilters.experienceTo) {
          return showExperience(
            selectedFilters.experienceFrom,
            selectedFilters.experienceTo,
            false
          );
        }
        return filter;
                    ...
                    {filter === "Опыт" && (
                      <div className={styles.experienceFilter}>
                        ...
                        <button
                          onClick={() =>
                            applyFilter("Опыт", {
                              from: selectedFilters.experienceFrom,
                              to: selectedFilters.experienceTo
                            })
                          }
